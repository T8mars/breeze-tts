...
def test_supported_templates_match_pytorch_prompt_layouts() -> None:
    ...
    instruction_request = {**plain_request, "instruction": "Speak warmly."}
    clone_request = {
        **plain_request,
        "ref_audio_path": "/tmp/ref.wav",
        "ref_text": "reference",
    }
    direction_request = {**clone_request, "instruction": "Speak warmly."}
    ...
    assert get_template("tts_instruction").build_segments(instruction_request) == [
        {"type": "text", "text": "[S0]<ins_bos>Speak warmly.<ins_eos>target"}
    ]
