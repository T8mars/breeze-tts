...
The build creates a private portable CPython 3.10 runtime, installs official Breeze runtime requirements with PyTorch 2.9.1 CUDA 12.8, `faster-whisper` 1.2.1, and a pinned Whisper Small checkpoint, verifies exact versions, builds Electron, and writes SHA-256 checksums. The default Whisper Small transcription path is offline at `resources/backend/models/faster-whisper-small`; in the packaged desktop app, other Whisper sizes download on first use into `%APPDATA%\T8star-Aix Voice Studio\models\whisper`. Because the bundled CUDA runtime exceeds Squirrel's reliable embedded-Setup size, the release uses a verified 7-Zip self-extracting EXE and manual update manifest. The application downloads the fixed Breeze model revision only after the user accepts its license. Version 0.2.9 adds separate GitHub, Hugging Face, Bilibili, and YouTube creator links and retains runnable batch examples, vocal-event/role detection, Voice Library reset, reference upload/playback, editable timelines, per-line rerun, inline vocal events, history/queue recovery, secure bundles, SRT, Fast/Eager, and Transformers 4.57.x/5.x compatibility.
